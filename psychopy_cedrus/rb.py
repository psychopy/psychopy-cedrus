...
class RBPhotodiodeGroup(photodiode.BasePhotodiodeGroup):
    def __init__(self, pad, channels=1):
        # get parent
        self.parent = RB.resolve(pad)
    ...
    def isSameDevice(self, other):
        ...
        Parameters
        ----------
        other : RBPhotodiodeGroup, dict
            Other RBPhotodiodeGroup to compare against, or a dict of params (which must include
            `index` as a key)
        ...
        if isinstance(other, type(self)):
            # if given another RBPhotodiodeGroup, compare parent boxes
            other = other.parent
    ...
    def getAvailableDevices():
        ...
        for profile in RB.getAvailableDevices():
            devices.append({
    ...
    def dispatchMessages(self):
        """
        Dispatch messages from parent RB to this photodiode group
...
class RBButtonGroup(button.BaseButtonGroup):
    ...
    def isSameDevice(self, other):
        ...
        Parameters
        ----------
        other : RBButtonGroup, dict
            Other RBButtonGroup to compare against, or a dict of params (which must include
            `index` as a key)
        ...
        """
        if isinstance(other, type(self)):
            # if given another RBButtonGroup, compare parent boxes
            other = other.parent
        elif isinstance(other, dict) and "pad" in other:
            # if given a dict, make sure we have a `port` rather than a `pad`
            other['port'] = other['pad']
    ...
    def dispatchMessages(self):
        """
        Dispatch messages from parent RB to this button group
...
class RB(base.BaseDevice):
    def __init__(
            self, index=0
    ):
        # give error if no device connected
        if not len(self.getAvailableDevices()):
            raise ConnectionError("No Cedrus RB response pad is connected.")
        # get xid device
        self.index = index
    ...
    def resolve(cls, requested):
        """
        Take a value given to a device which has a RB as its parent and, from it, 
        find/make the associated RB object.

        Parameters
        ----------
        requested : str, int or RB
            Value to resolve
        """
        ...
        if isinstance(requested, int):
            pad = DeviceManager.getDeviceBy("index", requested, deviceClass="psychopy_cedrus.rb.RB")
            # if found, return
            if pad is not None:
                return pad
        ...
        if requested is None or isinstance(requested, int):
            return DeviceManager.addDevice(
                deviceClass="psychopy_cedrus.rb.RB",
                deviceName=f"RB@{requested}",
                index=requested
            )
        # if still not found, raise error
        raise ManagedDeviceError(f"Could not find/create any RB object from the value {requested}")
    ...
    def isSameDevice(self, other):
        ...
        ----------
        other : RB, dict
            Other RB to compare against, or a dict of params (which much include
            `index` as a key)
    ...
    def addListener(self, listener):
        """
        Add a listener, which will receive all the messages dispatched by this RB.

        Parameters
        ----------
        listener : hardware.listener.BaseListener
            Object to duplicate messages to when dispatched by this RB.
        """
    ...
    def dispatchMessages(self):
        ...
        while self.xid.has_response():
            ...
            for node in self.nodes:
                ...
                if resp['port'] == 0 and not isinstance(node, RBButtonGroup):
                    continue
                # if device is 3, dispatch only to photodiodes
                if resp['port'] == 3 and not isinstance(node, RBPhotodiodeGroup):
                    continue
                # if device is 2, dispatch only to voice keys
                if resp['port'] == 2 and not isinstance(node, RBVoicekey):
                    continue
                # dispatch to node
